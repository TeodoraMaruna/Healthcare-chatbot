...
print(data_summarized1)
print(data_summarized2)
...
f4 = open('./dataset/intents.json')             #  6 124
...
for batch in batched(data_intents, 4):
    ...
    # encoder_inp = tokenizer.texts_to_sequences(X_train)
    # decoder_inp = tokenizer.texts_to_sequences(y_train)
    ...
    isFile = os.path.isfile("chatbot_incremental_learning.h5")
    if isFile:
        model.load_weights('chatbot_incremental_learning.h5')
        
    model.fit([encoder_inp, decoder_inp], decoder_final_output, epochs=100, batch_size=24, validation_split=0.15)
    model.save('chatbot_incremental_learning.h5')
    model.save_weights('chatbot_incremental_learning.h5')
    
    # model.load_weights('chatbot_weights_doctor_glove_intents.h5')
...
#     print('Bleu score: ', mean(bleu_score_list))   
