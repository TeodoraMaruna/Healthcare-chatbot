...
f = open('conversation_dataset_without_intents_train.json')
data = json.load(f)
print(len(data))
...
inv_vocab = {w:v for v, w in vocab.items()}
    ...
    # treniranje modela
counter = 0
for x in constrained_batches(data, max_size=10000000, max_count = 4050):
    ...
    if counter != 0:
        ...
        model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc'])
        checkpoint_callback = ModelCheckpoint('chatbot_incremental_learning_best_weights20.h5', save_best_only=True, save_weights_only=True, monitor='val_loss', mode='min')
        history = model.fit([encoder_inp, decoder_inp], decoder_final_output, epochs=85 + n, batch_size=24, callbacks=[checkpoint_callback],validation_split=0.15, initial_epoch = n)
        n = n + 100
    else:
        checkpoint_callback = ModelCheckpoint('chatbot_incremental_learning_best_weights10.h5', save_best_only=True, save_weights_only=True, monitor='val_loss', mode='min')
...
model.save('chatbot_incremental_learning_without_intents.h5')
model.save_weights('chatbot_incremental_learning_without_intents.h5') 
  
print("hi")
model.load_weights('chatbot_incremental_learning_without_intents.h5')    
...
plt.savefig('acc_incremental.svg')
plt.show()
...
plt.xlabel('epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig('loss_incremental.svg')
plt.show()
...
def evaluate():
    ...
    X_test = []
    y_test = []
    f = open('conversation_dataset_without_intents_test.json')
    data = json.load(f)
    for pair in data:
